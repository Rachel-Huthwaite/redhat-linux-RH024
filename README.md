# Red Hat Linux Administration — Learning Journey (RH024)
My public learning log for Red Hat's RH024 course: lesson breakdowns, a growing Linux command/jargon glossary, and real Fedora troubleshooting stories.

## How to read this repo

1. Start with [`PersonalMotivation/`](./PersonalMotivation) — why I'm doing
   this, and my actual lived experience switching to Linux (Fedora) before
   starting the course.
2. [`LessonBreakdowns/`](./LessonBreakdowns) — a structured summary of every
   video in the course, one file per lesson.
3. [`CommandsAndJargon/`](./CommandsAndJargon) — a running glossary of every
   command and term I pick up.
4. [`FunLearning/`](./FunLearning) — self-made quizzes and exercises, the
   study method that's worked best for me since high school.


## Course progress checklist

> Updated as I move through the RH024 videos.

| # | Lesson | Status | Breakdown |
|---|--------|--------|-----------|
| 01 | Linux Distributions | ✅ Complete| [01-linux-distributions.md](./LessonBreakdowns/01-linux_distributions.md) |
| 02 | Introduction to Shell | ✅ Complete| [02-introduction-to-shell.md](./LessonBreakdowns/02-introduction-to-shell.md) |
| 03 | Documentation | ✅ Complete| [03-documentation.md](./LessonBreakdowns/03-documentation.md) |
| 04 | Command Line Assistant | ✅ Complete| [04-command-line-assistant.md](./LessonBreakdowns/04-command-line-assistant.md) |
| 05 | Linux Directories Explained | 🟡 In progress| [05-linux-directories-explained.md](./LessonBreakdowns/05-linux-directories-explained.md) |
| 06 | Basic File Management | 🔲 Not started| [06-basic-file-management.md](./LessonBreakdowns/06-basic-file-management.md) |
| 07 | Editing Files with Vim | 🔲 Not started| [07-editing-filws-with-vim.md](./LessonBreakdowns/07-editing-filws-with-vim.md) |
| 08 | Organizing Local Users and Groups | 🔲 Not started| [08-organizing-local-users-and-groupds.md](./LessonBreakdowns/08-organizing-local-users-and-groupds.md) |
| 09 | File Permissions | 🔲 Not started| [09-file-permissions.md](./LessonBreakdowns/09-file-permissions.md) |
| 10 | Managing Software and Updates | 🔲 Not started| [10-managing-software-and-updates.md](./LessonBreakdowns/10-managing-software-and-updates.md) |
| 11 | Managing Networking | 🔲 Not started| [11-managing-networking.md](./LessonBreakdowns/11-managing-networking.md) |
| 12 | Managing System Startup Services with Systemd | 🔲 Not started| [12-managing-system-startup-services-with-systemd.md](./LessonBreakdowns/12-managing-system-startup-services-with-systemd.md) |
| 13 | Deployong an application runtime to host a simple application | 🔲 Not started| [13-deploying-an-application-runtime-to-host-a-simple-application.md](./LessonBreakdowns/13-deploying-an-application-runtime-to-host-a-simple-application.md) |
| 14 | Using Image Mode with Bootc | 🔲 Not started| [14-using-image-mode-with-bootc.md](./LessonBreakdowns/14-using-image-mode-with-bootc.md) |
| 15 | Image Builder | 🔲 Not started| [15-image-builder.md](./LessonBreakdowns/15-image-builder.md) |


*(More rows added as I progress through the course.)*

**Legend:** 🔲 Not started · 🟡 In progress · ✅ Complete

## Skills built so far

*(This section grows as the course progresses — a running summary of what
I can actually do as a result of this repo, not just what I've watched.)*

## Repository verification

WTC-7SQT9X9B
